...
## 3. Configuration
**Exercise 2-4** uses a `config.yml` file for customizable parameters like colors, drawing thickness, and room detection thresholds (e.g., `min_room_area_px`). It also stores the Regular Expression (`area_regex`) used to match area labels. You can easily modify these parameters without changing the python code.
